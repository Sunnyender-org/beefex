...
if (isMain) {
  ...
  const version = take(args, '--version') || '0.1.0-alpha.5'
  const commit = take(args, '--commit') || ''
  const sumsPath = take(args, '--sha256sums')
  if (!sumsPath) {
    throw new Error('usage: build-beefex-updater-metadata.mjs --sha256sums SHA256SUMS.txt [--version 0.1.0-alpha.5] [--commit <release-commit>]')
  }
