    ...
    pub async fn run_turn(
        ...
        loop {
            match control.try_recv() {
                ...
                Ok(SessionCommand::Rpc { response, .. }) => {
                    let _ = response.send(Err("RPC command is only supported by Pi".to_string()));
                }
...
mod tests {
    ...
    fn event_variant(event: &UnifiedAgentEvent) -> &'static str {
        match event {
            ...
            UnifiedAgentEvent::PiToolResult { .. } => "PiToolResult",
            ...
            UnifiedAgentEvent::RuntimeStatus { .. } => "RuntimeStatus",
        }
    }
