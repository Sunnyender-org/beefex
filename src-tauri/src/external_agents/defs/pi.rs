...
mod tests {
    ...
    #[test]
    fn pi_build_args_resume_the_pi_owned_session_file() {
        let args = build_pi_args(
            &RuntimeContext {
                extra_allowed_dirs: Vec::new(),
                resume_session_id: Some("/tmp/beefex-pi/session.jsonl".to_string()),
                new_session_id: None,
                include_partial_messages: false,
            },
            &RuntimeBuildOptions {
        ...
        assert!(args
            .windows(2)
            .any(|pair| pair == ["--session", "/tmp/beefex-pi/session.jsonl"]));
    }
}
