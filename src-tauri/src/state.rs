...
use crate::beefapi::account::AccountService;
...
#[serde(rename_all = "camelCase")]
pub struct ChatToolApprovalIdentity {
...
#[derive(Debug)]
pub struct PendingChatToolApproval {
    pub identity: ChatToolApprovalIdentity,
    pub generation: u64,
    pub message_id: String,
    pub summary: String,
...
pub struct AppState {
    ...
    /// 每条 Chat run 的运行态。审批必须同时匹配 conversation/run/tool 且 run 仍活跃。
    pub chat_run_states: Mutex<HashMap<(String, String), ChatRunRuntime>>,
    /// 等待用户确认的敏感 Chat tool 调用，按完整 conversation/run/tool 身份隔离。
    pub pending_chat_tool_approvals:
        Mutex<HashMap<ChatToolApprovalIdentity, PendingChatToolApproval>>,
    ...
    pub external_agent_models_cache: Mutex<
        ...
        >,
    >,
    /// 外部 CLI 全量检测结果缓存（cwd → available/version/auth/models）。避免设置页或诊断流程
    ...
    pub http: Client,
    pub(crate) beefapi_account: Arc<AccountService>,
...
impl AppState {
    ...
    pub(crate) fn base(
        ...
        usage_dir: PathBuf,
        http: Client,
        beefapi_account: Arc<AccountService>,
        ...
        AppState {
            ...
            chat_run_states: Mutex::new(HashMap::new()),
    ...
    pub fn begin_chat_run(
        ...
        let mut terminal = runs
            .iter()
            .filter(|((task, _), run)| task == conversation_id && run.status.is_terminal())
            .map(|(key, run)| (key.clone(), run.started_at))
            .collect::<Vec<_>>();
        terminal.sort_by_key(|(_, started_at)| *started_at);
        for (key, _) in terminal.into_iter().rev().skip(31) {
            runs.remove(&key);
        }
        runs.insert(
            (conversation_id.to_string(), run_id.to_string()),
            ChatRunRuntime {
    ...
    pub fn set_chat_run_status(&self, conversation_id: &str, run_id: &str, status: ChatRunStatus) {
        let changed = {
            let mut runs = self
                .chat_run_states
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            let Some(run) = runs.get_mut(&(conversation_id.to_string(), run_id.to_string())) else {
                return;
            };
        ...
        if changed {
            self.deny_pending_chat_tool_approvals(conversation_id, Some(run_id));
        }
    }

    pub fn chat_run_status(&self, conversation_id: &str, run_id: &str) -> Option<ChatRunStatus> {
        self.chat_run_states
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .get(&(conversation_id.to_string(), run_id.to_string()))
            .map(|run| run.status)
    }

    pub fn latest_chat_run_status(&self, conversation_id: &str) -> Option<(String, ChatRunStatus)> {
        self.chat_run_states
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .iter()
            .filter(|((task_id, _), _)| task_id == conversation_id)
            .max_by_key(|(_, run)| (run.started_at, run.generation))
            .map(|((_, run_id), run)| (run_id.clone(), run.status))
    }

    pub fn finish_chat_run_from_guard(&self, conversation_id: &str, run_id: &str, generation: u64) {
        let status = if self.is_chat_generation_active(conversation_id, generation) {
            ChatRunStatus::Failed
        } else {
            ChatRunStatus::Cancelled
        };
        self.set_chat_run_status(conversation_id, run_id, status);
    }

    pub fn register_chat_tool_approval(
        &self,
        ...
        let can_register = {
            let mut runs = self
                ...
                .unwrap_or_else(|e| e.into_inner());
            let Some(run) =
                runs.get_mut(&(identity.conversation_id.clone(), identity.run_id.clone()))
            else {
                return false;
            };
            if run.generation != generation || run.status != ChatRunStatus::Running {
                return false;
            }
        ...
        if let Some(previous) = previous {
            let _ = previous.sender.send(false);
        }
        true
    }

    pub fn resolve_chat_tool_approval(
        &self,
        identity: &ChatToolApprovalIdentity,
        approved: bool,
    ) -> Result<(), String> {
        ...
        let mut pending = self
            ...
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let run = runs.get_mut(&(identity.conversation_id.clone(), identity.run_id.clone()));
        if !pending.contains_key(identity) {
            let mismatched = pending.keys().any(|candidate| {
                candidate.tool_call_id == identity.tool_call_id
                    || (candidate.conversation_id == identity.conversation_id
                        && candidate.run_id == identity.run_id)
            });
            if mismatched {
                return Err("approval_identity_mismatch".to_string());
            }
            return match run.map(|run| run.status) {
                Some(ChatRunStatus::Cancelled) => Err("run_cancelled".to_string()),
                Some(status) if status.is_terminal() => Err("run_not_active".to_string()),
                _ => Err("approval_not_found".to_string()),
            };
        }
        let Some(run) = run else {
            return Err("run_not_active".to_string());
        };
        if run.status == ChatRunStatus::Cancelled {
            return Err("run_cancelled".to_string());
        }
        if run.status != ChatRunStatus::AwaitingApproval {
            return Err("run_not_active".to_string());
        }
        let generation = pending
            .get(identity)
            .map(|approval| approval.generation)
            .ok_or_else(|| "approval_not_found".to_string())?;
        let generation_active = self
            .chat_active_generations
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .get(&identity.conversation_id)
            .is_some_and(|active| active.contains(&generation));
        if run.generation != generation || !generation_active {
            return Err("run_not_active".to_string());
        }
    ...
    pub fn remove_chat_tool_approval(&self, identity: &ChatToolApprovalIdentity) {
        {
            let mut runs = self
                .chat_run_states
                .lock()
        ...
        self.pending_chat_tool_approvals
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .remove(identity);
    }

    fn deny_pending_chat_tool_approvals(&self, conversation_id: &str, run_id: Option<&str>) {
        let senders = {
            let mut pending = self
                .pending_chat_tool_approvals
                .lock()
            ...
            let identities = pending
                ...
                .filter(|identity| {
                    identity.conversation_id == conversation_id
                        && run_id.is_none_or(|run_id| identity.run_id == run_id)
                })
                .cloned()
                .collect::<Vec<_>>();
            identities
                .into_iter()
                .filter_map(|identity| pending.remove(&identity).map(|approval| approval.sender))
                .collect::<Vec<_>>()
        };
    ...
    pub fn cancel_chat_generation(&self, conversation_id: &str) {
        {
            let mut runs = self
                ...
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            for ((task_id, _), run) in runs.iter_mut() {
                if task_id == conversation_id && !run.status.is_terminal() {
                    run.status = ChatRunStatus::Cancelled;
                }
            }
        }
        self.deny_pending_chat_tool_approvals(conversation_id, None);
        self.chat_active_generations
            .lock()
            .unwrap_or_else(|e| e.into_inner())
    ...
    pub fn forget_chat_conversation_runtime(&self, conversation_id: &str) {
        ...
        self.chat_run_states
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .retain(|(task_id, _), _| task_id != conversation_id);
        self.chat_active_generations
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .remove(conversation_id);
    ...
    pub fn pi_live_session_control(
        &self,
        conversation_id: &str,
        cwd: &str,
    ) -> Option<tokio::sync::mpsc::Sender<crate::external_agents::session::live::SessionCommand>>
    {
        ...
        if let Some(session) = map.get_mut(conversation_id) {
            if !session.control.is_closed()
                && session.agent_id.starts_with("pi@")
                && session.cwd == cwd
            {
                session.last_activity = Instant::now();
                return Some(session.control.clone());
...
mod tests {
    ...
    #[test]
    fn latest_run_uses_generation_when_timestamps_are_equal() {
        let st = test_state();
        let gen_a = st.next_chat_generation("conv");
        let gen_b = st.next_chat_generation("conv");
        st.begin_chat_run("conv", "run-a", "message-a", gen_a);
        st.begin_chat_run("conv", "run-b", "message-b", gen_b);
        {
            let mut runs = st
                .chat_run_states
                .lock()
                .unwrap_or_else(|error| error.into_inner());
            runs.get_mut(&("conv".to_string(), "run-a".to_string()))
                .expect("first run")
                .started_at = 42;
            runs.get_mut(&("conv".to_string(), "run-b".to_string()))
                .expect("second run")
                .started_at = 42;
        }

        assert_eq!(
            st.latest_chat_run_status("conv"),
            Some(("run-b".to_string(), ChatRunStatus::Running))
        );
    }
