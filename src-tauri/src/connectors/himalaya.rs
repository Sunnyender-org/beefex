...
fn beefex_himalaya_install_dir() -> Option<PathBuf> {
    crate::app_paths::app_data_dir().map(|dir| dir.join("himalaya-cli"))
}

fn beefex_himalaya_binary_path() -> Option<PathBuf> {
    let dir = beefex_himalaya_install_dir()?;
    #[cfg(windows)]
    {
...
pub fn resolve_himalaya_binary() -> Option<PathBuf> {
    beefex_himalaya_binary_path().or_else(which_himalaya_on_path)
}
...
pub fn beefex_himalaya_path_env_when_active(
    accounts: &[EmailAccountConfig],
) -> Option<(String, std::ffi::OsString)> {
    if accounts.is_empty() {
        None
    } else {
        beefex_himalaya_path_env()
    }
}

/// If Beefex manages a himalaya install, return its directory (for PATH prepend).
pub fn beefex_himalaya_bin_dir() -> Option<PathBuf> {
    beefex_himalaya_binary_path().and_then(|path| path.parent().map(Path::to_path_buf))
}

/// Returns `(PATH env key, merged value)` when Beefex manages a himalaya install.
pub fn beefex_himalaya_path_env() -> Option<(String, std::ffi::OsString)> {
    let dir = beefex_himalaya_bin_dir()?;
...
pub async fn install_himalaya(http: &reqwest::Client) -> Result<HimalayaInstallResult, String> {
    ...
    let install_dir = beefex_himalaya_install_dir()
...
pub fn render_himalaya_config_toml(accounts: &[EmailAccountConfig]) -> String {
    let mut out = String::from(
        "# Generated by Beefex — do not edit manually if you use Beefex email connector.\n\n",
    );
...
mod tests {
    ...
    fn render_config_empty_accounts_has_no_account_sections() {
        ...
        assert!(toml.contains("Generated by Beefex"));
    }

    #[test]
    fn merged_shell_path_prepends_directory() {
        let merged = merged_shell_path_with_dir(Path::new("/beefex/himalaya-cli"));
        let merged = merged.to_string_lossy();
        assert!(merged.starts_with("/beefex/himalaya-cli"));
    }
}
