...
impl OpenAiResponsesProvider<'_> {
    ...
    fn request_body(&self, request: &GenerateRequest, stream: bool) -> Value {
        ...
        if let Some(key) = crate::settings::is_beefapi_url(&self.provider.base_url)
            .then_some(request.metadata.conversation_id.as_deref())
            .flatten()
...
fn prompt_cache_key(conversation_id: &str) -> Option<String> {
    ...
    Some(format!(
        "{:x}",
        Sha256::digest(format!("beefex:{conversation_id}"))
    ))
}
...
mod tests {
    ...
    #[test]
    fn request_body_uses_a_stable_bounded_prompt_cache_key_per_conversation() {
        let state = crate::state::AppState::new_headless(
            crate::settings::Settings::default(),
            std::env::temp_dir(),
        );
        let provider = ModelProvider {
            ...
            api_keys: vec!["sk-test".into()],
            api_key_legacy: None,
            base_url: "https://beefapi.com/v1".into(),
            available_models: vec!["gpt-5.6-sol".into()],
            enabled_models: vec!["gpt-5.6-sol".into()],
            enabled: true,
            api_format: "openai_responses".into(),
            model_overrides: std::collections::HashMap::new(),
            compress_request_body: false,
        };
        let request = |conversation_id: &str| GenerateRequest {
        ...
        let first = adapter.request_body(&request("conv_abc"), false);
        let repeated = adapter.request_body(&request("conv_abc"), true);
        let other = adapter.request_body(&request("conv_other"), false);
        let mut overridden_request = request("conv_abc");
        overridden_request.options.provider_options = serde_json::json!({
            "prompt_cache_key": "caller-controlled-key"
        });
        let overridden = adapter.request_body(&overridden_request, false);

        assert_eq!(first["prompt_cache_key"], repeated["prompt_cache_key"]);
        assert_ne!(first["prompt_cache_key"], other["prompt_cache_key"]);
        assert_eq!(first["prompt_cache_key"], overridden["prompt_cache_key"]);
        assert_eq!(
            first["prompt_cache_key"],
            "20fef0c49aacbbe9f6831e1c89f66aeeeaebf80f1a9d8727ee7fc736304dedd0"
        );
        assert!(first["prompt_cache_key"].as_str().unwrap().len() <= 64);
    }

    #[test]
    fn request_body_omits_prompt_cache_key_without_a_conversation() {
        let state = crate::state::AppState::new_headless(
            crate::settings::Settings::default(),
            std::env::temp_dir(),
        );
        ...
        assert!(OpenAiResponsesProvider::new(&state, &provider, 1)
            .request_body(&request, false)
            .get("prompt_cache_key")
            .is_none());

        let mut whitespace_request = request.clone();
        whitespace_request.metadata.conversation_id = Some("   ".into());
        assert!(OpenAiResponsesProvider::new(&state, &provider, 1)
            .request_body(&whitespace_request, false)
            .get("prompt_cache_key")
            .is_none());

        let mut third_party_provider = provider;
        third_party_provider.base_url = "https://api.openai.com/v1".into();
        let mut third_party_request = request;
        third_party_request.metadata.conversation_id = Some("conv_abc".into());
        assert!(
            OpenAiResponsesProvider::new(&state, &third_party_provider, 1)
                .request_body(&third_party_request, false)
                .get("prompt_cache_key")
                .is_none()
        );
    }
    ...
    #[test]
    fn stream_conformance_preserves_reasoning_tool_usage_and_terminal_order() {
        let events = vec![
            serde_json::json!({ "type": "response.output_text.delta", "delta": "先检查" }),
            serde_json::json!({ "type": "response.reasoning_summary_text.delta", "delta": "需要读取文件" }),
            serde_json::json!({
                "type": "response.output_item.added",
            ...
            serde_json::json!({
                "type": "response.output_item.done",
                "item": { "id": "fc_1", "type": "function_call", "call_id": "call_1", "name": "read", "arguments": "{\"path\":\"README.md\"}" }
            }),
            serde_json::json!({
                "type": "response.completed",
        ...
        let order = parts
            ...
            .map(|part| match part {
                ...
                StreamPart::ReasoningDelta { .. } => "reasoning",
                StreamPart::ToolCallStart { .. } => "tool_start",
                StreamPart::ToolCallDelta { .. } => "tool_delta",
                StreamPart::ToolCallDone { .. } => "tool_done",
        ...
        assert_eq!(output.usage.and_then(|usage| usage.total_tokens), Some(13));
        assert_eq!(output.finish_reason.as_deref(), Some("tool_calls"));
    }
