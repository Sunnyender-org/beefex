...
use crate::chat::model_call::{
    chat_missing_model_error, format_chat_missing_api_key_error, resolve_model_provider,
...
use crate::chat::storage::{find_set_by_id, save_conversation};
...
use crate::chat::{AgentRuntimeConfig, AgentRuntimeKind, Conversation, ToolCallStatus};
use crate::skills;
...
pub(super) async fn complete_assistant_reply_inner(
    ...
    pin_managed_pi_runtime(conversation);
    if conversation.agent_runtime.is_external() {
        if arm.is_some() {
            return Err("external_runtime_fan_out_unsupported".to_string());
        }
        ...
        state.begin_chat_run(&conversation.id, &ext_run_id, "", ext_generation);
        apply_active_run_projection(conversation, &ext_run_id);
        ...
        let latest_user = conversation
            ...
            .unwrap_or_default();
        let result = crate::external_agents::run_external_cli_reply(
            app,
            state,
            conversation,
            title_from_first_user,
            ...
            active_skill_id,
            entry,
            Some(crate::external_agents::run::ExternalRunIdentity {
                run_id: ext_run_id.clone(),
        ...
        match &result {
            Ok(outcome) => {
                ...
                save_conversation(app, conversation)?;
                state.set_chat_run_status(&conversation.id, &outcome.run_id, outcome.status);
            }
            Err(_) => {
                apply_terminal_run_projection(
                    conversation,
                    ...
                    crate::chat::ChatCompletionReceipt::default(),
                );
                save_conversation(app, conversation)?;
                state.set_chat_run_status(&conversation.id, &ext_run_id, ChatRunStatus::Failed);
            }
        }
        return result.map(|_| ArmReplyOutcome { message: None });
    }
    ...
    let managed_provider = crate::beefapi::provider::is_managed_provider_id(&resolved_provider_id);
    ...
    if arm.is_none() {
        // Crash marker for the single disk-owning run. A relaunch sanitizes this
        // non-terminal projection to `interrupted`; fan-out arms still avoid
        // concurrent conversation writes and are merged by their coordinator.
        apply_active_run_projection(conversation, &run_id);
        save_conversation(app, conversation)?;
    }
    ...
    if !plan_mode && model_can_generate_images_directly(&provider, &resolved_model) {
        ...
        let result = complete_direct_image_generation_reply(
            app,
            state,
            &settings,
            &provider,
        ...
        if result.is_ok() {
            ...
            save_conversation(app, conversation)?;
            state.set_chat_run_status(&conversation.id, &run_id, ChatRunStatus::Completed);
        }
        return result.map(|_| ArmReplyOutcome { message: None });
    }
    ...
    } else if managed_provider {
        effective_chat_tools.approval_policy = "scoped_coding_confirm".to_string();
    }
    ...
    let result = match result {
        Err(error)
            if managed_provider
                && matches!(crate::api::extract_status_code(&error), Some(401 | 403)) =>
        {
    ...
    let completion_receipt = crate::chat::receipt::build_completion_receipt(&tool_records);
    let terminal_status = chat_run_status_for_outcome(&result.stream_outcome);
    ...
    if let Some(arm) = arm {
        ...
        apply_terminal_run_projection(conversation, &run_id, terminal_status, completion_receipt);
        state.set_chat_run_status(&conversation.id, &run_id, terminal_status);
...
pub(super) fn managed_pi_runtime() -> AgentRuntimeConfig {
    AgentRuntimeConfig {
        kind: AgentRuntimeKind::External,
        external_agent_id: Some("pi".to_string()),
        external_model: Some("default".to_string()),
        external_reasoning: None,
        external_sandbox: None,
    }
}

pub(super) fn pin_managed_pi_runtime(conversation: &mut Conversation) {
    conversation.provider_id = crate::beefapi::provider::MANAGED_PROVIDER_ID.to_string();
    conversation.agent_runtime = managed_pi_runtime();
}

fn chat_run_status_for_outcome(outcome: &str) -> ChatRunStatus {
    match outcome {
...
fn apply_active_run_projection(conversation: &mut Conversation, run_id: &str) {
    conversation.last_run_status = Some(ChatRunStatus::Running);
    conversation.last_run_id = Some(run_id.to_string());
    conversation.last_run_terminal_at = None;
    conversation.last_run_receipt = None;
}

fn apply_terminal_run_projection(
    conversation: &mut Conversation,
    run_id: &str,
    status: ChatRunStatus,
    receipt: crate::chat::ChatCompletionReceipt,
) {
    conversation.last_run_status = Some(status);
    conversation.last_run_id = Some(run_id.to_string());
    conversation.last_run_terminal_at = Some(chrono::Local::now().timestamp());
    conversation.last_run_receipt = Some(receipt);
}

pub(super) fn persist_latest_terminal_run_projection(
    app: &AppHandle,
    state: &AppState,
    conversation: &mut Conversation,
) -> Result<(), String> {
    let Some((run_id, status)) = state.latest_chat_run_status(&conversation.id) else {
        return Ok(());
    };
    if !status.is_terminal() {
        return Ok(());
    }
    if conversation.last_run_id.as_deref() == Some(run_id.as_str())
        && conversation.last_run_status == Some(status)
    {
        return Ok(());
    }
...
mod run_projection_tests {
    ...
    #[test]
    fn managed_beefapi_dispatch_runtime_is_pi() {
        let runtime = managed_pi_runtime();
        assert!(runtime.is_external());
        assert_eq!(runtime.external_agent_id.as_deref(), Some("pi"));
        assert_eq!(runtime.external_model.as_deref(), Some("default"));
    }

    #[test]
    fn historical_builtin_conversation_is_pinned_to_managed_pi_before_execution() {
        let mut conversation: Conversation = serde_json::from_value(serde_json::json!({
            "id": "conv_legacy",
            "title": "Legacy readable task",
        ...
        assert_eq!(
            ...
            crate::beefapi::provider::MANAGED_PROVIDER_ID
        );
        assert!(conversation.agent_runtime.is_external());
        assert_eq!(
            conversation.agent_runtime.external_agent_id.as_deref(),
            Some("pi")
        );
    }

    #[test]
    fn active_run_projection_replaces_stale_terminal_readback() {
        let mut conversation: Conversation = serde_json::from_value(serde_json::json!({
            "id": "conv_restart",
            "title": "Restart",
        ...
        apply_active_run_projection(&mut conversation, "run-current");

        assert_eq!(conversation.last_run_status, Some(ChatRunStatus::Running));
        assert_eq!(conversation.last_run_id.as_deref(), Some("run-current"));
        assert_eq!(conversation.last_run_terminal_at, None);
        assert!(conversation.last_run_receipt.is_none());
    }
}
