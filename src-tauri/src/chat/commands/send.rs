...
use super::reply::pin_managed_pi_runtime;
...
use super::{complete_assistant_reply, persist_latest_terminal_run_projection};
...
pub(crate) async fn chat_send_message(
    ...
    let fan_out = should_run_reply_fan_out(
        reply_arms.len(),
        plan_or_orchestrate,
        conversation.agent_runtime.is_external(),
    );
    ...
    if reply_outcome.is_err() {
        persist_latest_terminal_run_projection(&app, state.inner(), &mut conversation)?;
    }
...
fn should_run_reply_fan_out(
    reply_arm_count: usize,
    plan_or_orchestrate: bool,
    external_runtime: bool,
) -> bool {
    reply_arm_count >= 2 && !plan_or_orchestrate && !external_runtime
}

#[cfg(test)]
mod fan_out_routing_tests {
    use super::should_run_reply_fan_out;

    #[test]
    fn external_runtime_never_enters_multi_model_fan_out() {
        assert!(!should_run_reply_fan_out(2, false, true));
    }

    #[test]
    fn builtin_act_mode_keeps_multi_model_fan_out() {
        assert!(should_run_reply_fan_out(2, false, false));
        assert!(!should_run_reply_fan_out(1, false, false));
        assert!(!should_run_reply_fan_out(2, true, false));
    }
}
