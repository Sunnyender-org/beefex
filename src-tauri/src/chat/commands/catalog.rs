...
fn effective_new_conversation_model(state: &AppState, settings: &Settings) -> (String, String) {
    crate::beefapi::provider::managed_model_selection(&state.beefapi_account.state())
        .unwrap_or_else(|| settings.effective_chat_model())
}
...
pub(crate) fn create_chat_conversation_internal(
    ...
    let (default_provider_id, default_model) = effective_new_conversation_model(state, &settings);
...
pub(crate) fn chat_create_builder_conversation(
    ...
    let (default_provider_id, default_model) =
        effective_new_conversation_model(state.inner(), &settings);
    let provider_id = provider_id
        .and_then(non_empty_string)
        .unwrap_or(default_provider_id);
