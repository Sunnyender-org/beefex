...
fn inline_code_request_ignores_attachment_safe_copy_paths() {
    let content = compose_user_content_for_api(
            ...
            Some(Path::new("/Users/test/Library/Application Support/com.beefapi.beefex/conversations/conv_1_attachments")),
        );
...
#[test]
fn format_tool_approval_summary_marks_advisory_shell_risks() {
    let mut record = ToolCallRecord {
        id: "call_risk".to_string(),
        name: "bash".to_string(),
        source: "native".to_string(),
        server_id: None,
    ...
    record.arguments = r#"{"command":"curl https://example.com > /tmp/result.json"}"#.to_string();

    let summary = format_tool_approval_summary(&record);

    assert!(summary.contains("Advisory risks: network, redirection, absolute path"));
}
...
#[test]
fn format_pi_extension_approval_summary_hides_internal_envelope() {
    let record = ToolCallRecord {
        id: "call_pi_write".to_string(),
        name: "write".to_string(),
        ...
        structured_content: Some(serde_json::json!({
            ...
            "message": "internal envelope",
        })),
    };

    let summary = format_tool_approval_summary(&record);
    assert_eq!(summary, "Path: /tmp/project/out.txt");
    assert!(!summary.contains("Raw arguments"));
    assert!(!summary.contains("internal envelope"));
}
