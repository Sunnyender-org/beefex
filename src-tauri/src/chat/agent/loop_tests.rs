...
fn test_app_state() -> AppState {
    ...
    AppState::base(
        ...
        reqwest::Client::new(),
        std::sync::Arc::new(crate::beefapi::account::AccountService::unavailable()),
        #[cfg(target_os = "macos")]
...
fn responses_planning_tool_call_sse_events() -> Vec<String> {
    vec![
        r#"{"type":"response.output_item.added","item":{"id":"fc_1","type":"function_call","status":"in_progress","arguments":"","call_id":"call_read","name":"read"}}"#.to_string(),
        r#"{"type":"response.function_call_arguments.done","item_id":"fc_1","arguments":"{\"path\":\"README.md\"}"}"#.to_string(),
        r#"{"type":"response.output_item.done","item":{"id":"fc_1","type":"function_call","status":"completed","arguments":"{\"path\":\"README.md\"}","call_id":"call_read","name":"read"}}"#.to_string(),
        r#"{"type":"response.completed","response":{"status":"completed","usage":{"input_tokens":11,"output_tokens":4,"total_tokens":15}}}"#.to_string(),
    ]
}

fn responses_final_text_sse_events() -> Vec<String> {
    vec![
        r#"{"type":"response.output_text.delta","delta":"已读取并完成。"}"#.to_string(),
        r#"{"type":"response.completed","response":{"status":"completed","usage":{"input_tokens":19,"output_tokens":6,"total_tokens":25}}}"#.to_string(),
    ]
}
...
#[tokio::test]
async fn responses_run_loop_keeps_tool_call_result_and_terminal_in_one_state_machine() {
    let server = MockModelServer::start(vec![
        MockResponse::Sse(responses_planning_tool_call_sse_events()),
        MockResponse::Sse(responses_final_text_sse_events()),
    ]);
    let state = test_app_state();
    ...
    let result = run_agent_loop(config, &host, &executor)
        .await
        .expect("Responses tool loop completes");

    assert_eq!(result.content, "已读取并完成。");
    ...
    let requests = server.captured_bodies();
    assert_eq!(requests.len(), 2);
    let second: Value = serde_json::from_str(&requests[1]).expect("second Responses body");
    let input = second["input"].as_array().expect("Responses input array");
    let function_call = input
        .iter()
        .find(|item| item["type"] == "function_call")
        .expect("function_call replayed");
    let function_output = input
        .iter()
        .find(|item| item["type"] == "function_call_output")
        .expect("function_call_output replayed");
    assert_eq!(function_call["call_id"], "call_read");
    assert_eq!(function_output["call_id"], "call_read");
    assert!(function_output["output"]
        .as_str()
        .is_some_and(|output| output.contains("result:read")));
}
