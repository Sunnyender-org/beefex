...
pub async fn execute_tool_call(
    ...
    if super::prepare::tool_requires_session_consent(tool) {
        ...
        let scoped_read_only = policy == "scoped_coding_confirm"
            && crate::mcp::native_registry::find_entry(&tool.name)
                .is_some_and(|entry| entry.read_only);
        if scoped_read_only {
            // BeefAPI coding sessions keep navigation/read operations frictionless.
        } else if matches!(policy, "always_confirm" | "scoped_coding_confirm") {
...
mod tests {
    ...
    #[tokio::test]
    async fn scoped_coding_policy_auto_allows_read_only_project_tools() {
        let host = ExecuteTestHost::default();
        let executor = ExecuteTestExecutor::default();
        let mut settings = Settings::default();
        settings.chat_tools.approval_policy = "scoped_coding_confirm".to_string();
        let tool = crate::mcp::types::native_read_file_tool();
        let call = test_pending_call(
            "call_read",
            "read",
        ...
        assert!(matches!(record.status, ToolCallStatus::Success));
        assert_eq!(host.consents.load(Ordering::SeqCst), 0);
        assert_eq!(host.approvals.load(Ordering::SeqCst), 0);
        assert_eq!(executor.calls.load(Ordering::SeqCst), 1);
    }

    #[tokio::test]
    async fn scoped_coding_policy_confirms_each_file_mutation() {
        let host = ExecuteTestHost::default();
        let executor = ExecuteTestExecutor::default();
        let mut settings = Settings::default();
        settings.chat_tools.approval_policy = "scoped_coding_confirm".to_string();
        ...
        let call = test_pending_call(
            "call_write",
            "write",
            serde_json::json!({ "path": "src/demo.ts", "content": "export const ok = true\n" }),
        );
