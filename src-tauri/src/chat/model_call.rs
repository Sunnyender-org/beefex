...
pub(super) async fn resolve_model_provider(
    ...
    if crate::beefapi::provider::is_managed_provider_id(provider_id) {
        return state
            .beefapi_account
            .resolve_managed_provider(Some(model), |account_state| {
                if let Some(app) = app {
                    crate::beefapi::account::emit_account_state(app, &account_state);
                }
            })
            .await
            .map(|provider| provider.into_inner());
    }
    settings
