...
//! `~/.agent/`, not the project. Absolute paths and symlinks are confined to the
//! same project root; the user must open a different project to work elsewhere.
...
mod tests {
    ...
    #[tokio::test]
    async fn absolute_path_outside_project_is_rejected() {
        let (dir, executor) = temp_workspace();
        ...
        let error = executor
            ...
            .await
            .expect_err("project escape must fail closed");
        assert!(error.starts_with("project_path_outside_root:"));
