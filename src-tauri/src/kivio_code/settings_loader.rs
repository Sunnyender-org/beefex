//! Load the user's real Beefex `Settings` from the Tauri store on disk, without
//! a Tauri runtime.
//!
...
//! `app_data_dir` is derived from the bundle identifier `com.beefapi.beefex`:
//! - macOS:   `~/Library/Application Support/com.beefapi.beefex/`
//! - Linux:   `$XDG_DATA_HOME/com.beefapi.beefex/` (or `~/.local/share/...`)
//! - Windows: `%APPDATA%\com.beefapi.beefex\` (Roaming)
//!
...
/// Resolve `<app_data_dir>/settings.json` for the Beefex bundle identifier.
///
...
/// The per-app data directory Tauri would use for `com.beefapi.beefex`.
pub fn app_data_dir() -> Option<PathBuf> {
    // Must match Tauri's `app_data_dir` = `dirs::data_dir()/<identifier>` exactly:
    //   Windows: `%APPDATA%\com.beefapi.beefex`  (Roaming, NO `\data` subfolder)
    //   macOS:   `~/Library/Application Support/com.beefapi.beefex`
    //   Linux:   `$XDG_DATA_HOME/com.beefapi.beefex` (or `~/.local/share/...`)
    // NOTE: do NOT use `ProjectDirs::data_dir()` here — on Windows it appends a
    // `\data` subfolder (mac/Linux don't), so kivio-code would read
    // `...\com.beefapi.beefex\data\settings.json` and never find the GUI's file.
