...
use super::{resolve_tool_existing_dir, workspace_display_path, NativeToolWorkspace};
...
fn apply_project_shell_env(cmd: &mut Command) {
    cmd.env_clear();
    for key in [
        "PATH",
        "HOME",
...
pub async fn run_command(
    ...
    run_command_with_receipt(workspace, default_timeout_ms, arguments, state)
        .await
...
pub async fn run_command_with_receipt(
    workspace: &NativeToolWorkspace,
    default_timeout_ms: u64,
    arguments: &Value,
    state: Option<&AppState>,
    ...
    let lowered = requested_command.to_ascii_lowercase();
    ...
    let (command, cd_extracted) = normalize_run_command(&requested_command, explicit_cwd)?;
    ...
    let receipt_cwd = workspace_display_path(workspace, &cwd);
    ...
    if background {
        return run_shell_command_background(&command, cwd, state, workspace.has_project())
            .await
            .map(|content| CommandExecutionResult {
                content,
    ...
    let output =
        run_shell_command(&command, cwd, timeout_ms, state, workspace.has_project()).await?;
...
async fn run_shell_command_background(
    ...
    if minimal_env {
        apply_project_shell_env(&mut cmd);
    }
...
async fn run_shell_command(
    ...
    exec_shell_command(cmd, cwd, timeout_ms, state).await
}
...
mod tests {
    ...
    fn project_command_cwd_rejects_absolute_directory_outside_root() {
        ...
        let args = serde_json::json!({ "command": "pwd", "cwd": outside.to_string_lossy() });

        let error = resolve_command_cwd(&args, &workspace)
            .expect_err("project command cwd must stay inside project root");

        assert!(error.contains("project_path_outside_root"));
        let _ = std::fs::remove_dir_all(root);
    ...
    async fn project_command_receipt_records_exact_command_cwd_exit_and_stdout() {
        ...
        let result = run_command_with_receipt(
            &workspace,
            2_000,
        ...
        .await
        .expect("approved disposable project command");

        assert_eq!(result.command, "printf safe");
    ...
    #[cfg(not(target_os = "windows"))]
    #[tokio::test]
    async fn project_shell_environment_drops_unlisted_parent_secrets() {
        let mut command = build_shell_command("printf \"${BEEFEX_TEST_SECRET:-}\"");
        command.env("BEEFEX_TEST_SECRET", "must-not-leak");
        apply_project_shell_env(&mut command);
        command
        ...
        let output = command.output().await.expect("minimal project shell");
    ...
    fn normalize_run_command_rejects_cd_with_spaces() {
        let err = normalize_run_command("cd /Users/example/Project Files/foo && npm install", None)
            .expect_err("spaced cd path should require cwd");

        assert!(err.contains("Suggested cwd: /Users/example/Project Files/foo"));
