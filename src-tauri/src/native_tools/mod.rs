...
pub use shell::{run_command, run_command_with_receipt, CommandExecutionResult};
...
/// helpers below. Standalone conversations honor absolute/`~` paths; opened
/// projects are confined by `resolve_project_path`.
...
fn resolve_project_path(
    ...
    // Relative paths resolve against the project root. Absolute paths may be
    // supplied for model ergonomics, but canonical resolution must still land
    // inside the opened project. Canonicalizing the nearest existing parent
    // also closes symlink escapes for new files.
    let raw = Path::new(&expanded);
    ...
    if !resolved.starts_with(&root) {
        return Err(format!(
            "project_path_outside_root: {} is outside {}",
            resolved.display(),
            root.display()
        ));
    }
