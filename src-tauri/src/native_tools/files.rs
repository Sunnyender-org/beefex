...
mod tests {
    ...
    async fn simulated_agent_session_exercises_pi_style_tools() {
        ...
        .expect_err("write outside project must fail closed");
        println!("\n[4] write outside project -> {outside_error}");
        assert!(outside_error.contains("project_path_outside_root"));
        assert!(!outside.exists());

        // 5) an in-project write still succeeds and is readable through the same workspace.
        write_file(
            &ws,
            &json!({ "path": "notes/result.txt", "content": "stayed in the project root\n" }),
        )
        .expect("write inside project");
        let read = read_file(&ws, &json!({ "path": "notes/result.txt" })).expect("read back");
        println!("[5] read back content: {:?}", read.content.trim());
        assert_eq!(read.content, "stayed in the project root\n");
    ...
    fn project_workspace_confines_writes_and_symlink_escapes() {
        ...
        .expect_err("explicit absolute write outside project must fail");
        ...
        .expect_err("relative parent escape must fail");
        assert!(relative_err.contains("project_path_outside_root"));
        let escaped = root.parent().expect("root parent").join(&escape_name);
        assert!(!escaped.exists());

        #[cfg(unix)]
        {
            std::os::unix::fs::symlink(&dir, root.join("outside-link")).expect("symlink");
            let symlink_err = write_file(
                &workspace,
                &json!({ "path": "outside-link/note.txt", "content": "x" }),
            )
            .expect_err("symlink escape must fail");
            assert!(symlink_err.contains("project_path_outside_root"));
