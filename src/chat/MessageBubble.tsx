...
function withoutDuplicatedPersistedSynthesis(
  segments: ChatMessageSegment[],
  messageContent: string,
): ChatMessageSegment[] {
  const finalText = messageContent.trim()
  if (!finalText) return segments
  const matchingSynthesis = segments.find((segment) => (
    segment.kind === 'text'
    && segment.phase === 'synthesis'
    && (segment.text ?? '').trim() === finalText
  ))
  if (!matchingSynthesis) return segments

  return segments.filter((segment) => !(
    segment.kind === 'text'
    && segment.phase === 'tool_loop'
    && segment.order < matchingSynthesis.order
    && (segment.text ?? '').trim() === finalText
  ))
}
...
function MessageBubbleComponent({
  ...
  const timelineSegments = withoutDuplicatedPersistedSynthesis(
    orderedSegments(message.segments),
    message.content,
  )
