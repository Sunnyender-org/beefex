...
export const chatApi = {
  ...
  async previewPiProjectTrust(rootPath: string): Promise<PiProjectTrustPreview> {
    if (!isTauriRuntime()) {
      return {
        ...
        decision: 'trusted',
        inheritedFrom: rootPath,
        resources: [],
      }
    }
    const result = await invoke<{ success: boolean; trust: PiProjectTrustPreview }>(
      'chat_pi_project_trust_preview',
      { rootPath },
    )
    if (!result.success) throw new Error('Failed to inspect Pi project trust')
    return result.trust
  },

  async setPiProjectTrust(rootPath: string, trusted: boolean | null): Promise<PiProjectTrustPreview> {
    if (!isTauriRuntime()) {
      return {
        requestedPath: rootPath,
        trustPath: rootPath,
        isGitRepository: false,
        decision: trusted === true ? 'trusted' : trusted === false ? 'untrusted' : 'unknown',
        inheritedFrom: trusted == null ? null : rootPath,
        resources: [],
      }
    }
    const result = await invoke<{ success: boolean; trust: PiProjectTrustPreview }>(
      'chat_pi_set_project_trust',
      { rootPath, trusted },
    )
    if (!result.success) throw new Error('Failed to update Pi project trust')
    return result.trust
  },

  async piRpcCommand(conversationId: string, command: PiRpcCommand): Promise<unknown> {
    if (!isTauriRuntime()) return { preview: true, command: command.type }
    const result = await invoke<{ success: boolean; data: unknown }>('chat_pi_rpc_command', {
      conversationId,
      command,
    })
    if (!result.success) throw new Error(`Pi RPC ${command.type} failed`)
    return result.data
  },
