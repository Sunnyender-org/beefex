...
const PRIMARY_ROUTE_SHOWS_EXTENSIONS = false
...
function SidebarUserFooter({
  ...
  const accountReady = account.state.phase === 'signed_in'
  const accountOffline = account.state.phase === 'offline'
  const accountCanWork = canUseManagedBeefApiAccount(account.state)
  const hasAccountReadback = hasBeefApiAccountMetadata(account.state)
  const accountLabel = accountReady
    ? account.state.email || 'BeefAPI'
    : accountOffline
      ? 'BeefAPI 暂时离线'
      : '登录 BeefAPI'
  const accountDetail = accountCanWork
    ? `${formatAccountGroup(account.state.group)} · ${formatAccountModel(account.state.defaultModel)}`
    : accountOffline
      ? hasAccountReadback ? 'credential 已安全保留' : '尚未建立本机账号'
      : 'GPT Pro coding agent'

  const handleAccountAction = async () => {
    if (accountReady) {
      const next = await account.logout()
      if (next?.phase === 'signed_out') window.location.hash = '#chat/onboarding'
      return
    }
    if (accountOffline) {
    ...
    if (account.state.phase === 'cleanup_required') {
  ...
  return (
    <div
      className="relative shrink-0 border-t border-neutral-200/60 px-2 pb-2.5 pt-2 dark:border-neutral-800/80"
      data-tauri-drag-region="false"
    >
      {accountOpen ? (
        <div className="beef-account-popover" role="dialog" aria-label="BeefAPI 账号">
          <div className="beef-account-popover-head">
            {accountReady ? <CheckCircle2 size={15} /> : accountOffline ? <CloudOff size={15} /> : <ShieldAlert size={15} />}
            <div>
              <strong>{accountLabel}</strong>
              <span>{accountDetail}</span>
            </div>
          </div>
          {hasAccountReadback && account.state.email ? (
            <div className="beef-account-popover-email" title={account.state.email}>
              {account.state.email}
            </div>
          ) : null}
          <button
            ...
            className="beef-account-popover-action"
          >
            {accountReady ? <LogOut size={14} /> : accountOffline ? <RefreshCw size={14} /> : <ChevronRight size={14} />}
            {accountReady ? '退出 BeefAPI' : accountOffline ? '重试连接' : account.state.phase === 'cleanup_required' ? '管理 Token' : '打开登录'}
          </button>
        </div>
      ) : null}
      <div className="flex items-center gap-2 px-2 py-1.5">
        <button
          type="button"
          className="group flex min-w-0 flex-1 items-center gap-2 rounded-[5px] text-left active:scale-[0.98]"
          onClick={() => setAccountOpen((open) => !open)}
          aria-expanded={accountOpen}
          title={`${accountLabel} · ${accountDetail}`}
        >
          {profile.avatarUrl ? (
            <UserAvatar profile={profile} size={28} />
          ) : (
            <span className="beef-account-avatar" aria-hidden="true">
              {account.state.email?.trim().charAt(0) || 'B'}
            </span>
          )}
          ...
          <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${accountReady ? 'bg-[#6B8E3D] dark:bg-[#9CC267]' : accountOffline ? 'bg-amber-500' : 'bg-neutral-300 dark:bg-neutral-600'}`} />
        </button>
        <button
          type="button"
...
function formatAccountGroup(group: string | undefined) {
  return group?.toLowerCase() === 'gpt-pro' ? 'GPT Pro' : group || '未返回'
}

function formatAccountModel(model: string | undefined) {
  return model?.toUpperCase() || '未返回'
}
...
export const Sidebar = memo(function Sidebar({
  ...
  return (
    <>
      ...
      >
        <div
          className={`chat-sidebar-titlebar ${chatTitlebarRowClass} ${chatTitlebarMacInsetClass} pr-3`}
      ...
      <nav className="shrink-0 space-y-0.5 px-3 pb-2" data-tauri-drag-region="false">
        ...
        />
        {PRIMARY_ROUTE_SHOWS_EXTENSIONS ? (
          <ExtensionsNav
