      ...
      <SettingsGroup title={t('文档解析服务', 'Parsing service')}>
        <div className="px-1 py-2">
          <div className="kv-seg w-full">
            {[{ kind: '', name: t('Beefex 内置', 'Beefex built-in') }, ...THIRD_PARTY].map((s) => (
              <button
                key={s.kind || 'builtin'}
                type="button"
          ...
          <p className="kv-row-desc mt-1.5">
            {activeKind === ''
              ? t(
                  'Beefex 本地解析 txt / md / html / PDF 文字层 / docx / xlsx，免费离线。',
                  'Beefex parses txt, md, html, PDF text layer, docx and xlsx locally, free and offline.',
                )
              : t(
