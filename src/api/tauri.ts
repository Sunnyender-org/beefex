...
export type ChatPiRuntimeStatusPayload = {
  conversationId: string
  runId: string
  kind: string
...
function normalizeSettings(settings: Settings): Settings {
  ...
  return {
    ...
    autoCheckUpdate: current.autoCheckUpdate ?? false,
...
export type BeefApiAccountPhase =
  | 'signed_out'
  | 'authorizing'
  | 'polling'
  | 'signed_in'
...
/** Safe BeefAPI account projection. Credentials and device codes never cross IPC. */
export type BeefApiAccountState = {
  phase: BeefApiAccountPhase
  email?: string
  group?: string
  defaultModel?: string
...
export const api = {
  ...
  beefapiAuthStart: () => invoke<BeefApiAccountState>('beefapi_auth_start'),
  beefapiAuthCancel: () => invoke<BeefApiAccountState>('beefapi_auth_cancel'),
  beefapiAuthReopenBrowser: () => invoke<void>('beefapi_auth_reopen_browser'),
  beefapiLogout: () => invoke<BeefApiAccountState>('beefapi_logout'),
  onBeefapiAccountState: (listener: (state: BeefApiAccountState) => void) => {
  ...
  onChatPiRuntimeStatus: (listener: (payload: ChatPiRuntimeStatusPayload) => void) => {
    if (!isTauriRuntime()) return Promise.resolve(() => {})
    return on<ChatPiRuntimeStatusPayload>('chat-pi-runtime-status', (payload) => listener(payload))
  },
  ...
  chatConfirmToolCall: (
    ...
    toolCallId: string,
    approved: boolean,
  ) => invoke<void>('chat_confirm_tool_call', { conversationId, runId, toolCallId, approved }),
